...
def backtest_signal_validity(
    ...
    window: int = 21,
    lookback: int = 252 * 3,
    freq: str = "ME",   # ME = month-end (non-overlapping); "W-FRI" = weekly (overlapping)
):
    """
    ...
    print(f"  Trades: {n}")
    if n > 0:
        print(f"  Hit rate (overall): {hit:.2%}")
        print(f"  Mean Δcorr on trades: {trades['delta'].mean():.4f}")
        by_label = trades.groupby("label")["correct"].agg(["count", "mean"])
        print("  Hit rate by regime:")
        for lab_name, row in by_label.iterrows():
            print(f"    {lab_name:<5} n={int(row['count']):>3}  hit={row['mean']:.2%}")
    else:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--freq", default="ME", help="sampling frequency for validity test: ME (monthly, non-overlapping) or W-FRI (weekly, overlapping)")
    args = ap.parse_args()

    if args.backtest:
        backtest_signal_validity(args.index, args.components, args.start, args.window, args.lookback, freq=args.freq)
    else:
